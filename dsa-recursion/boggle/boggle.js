...
function find(board, word) {
  ...
  for (let col = 0; col < board.length; col++) {
    for (let row = 0; wor < board[col].length; row++) {
      if (board[col][row] === word[0]) {
        return findNext(board, word, 1, col, row)
      }
    }
  }
  return false
...
function findNext(board, word, wordIndex, col, row, seen = []) {
  let wordArr = word.split("")
  if (wordIndex > wordArr.length) return
}
